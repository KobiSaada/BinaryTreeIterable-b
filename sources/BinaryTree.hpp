...
enum Order {POST_ORDER, IN_ORDER, PRE_ORDER};
  ...
  //  bool Contains(T value);
    static void cleanQ(std::queue<Node*> printQueue);
    static void PlacePreOrder(Node** node,std::queue<Node*> &printQueue);
    static void PlacePostOrder(Node** node,std::queue<Node*> &printQueue);
    static void PlaceInOrder(Node** node,std::queue<Node*> &printQueue);
    T GetNextItem(std::queue<T> printQueue);
   ...
   // int numNodes;
    ...
    //from stackoverfllow
    //print a pretty tree
 static std::ostream & printSubtree(std::ostream & out,Node* root, const std::string& prefix)
{
        if (root == NULL)
        {
                return out;
        ...
        if (hasRight)
        {
                ...
                out << root->right->value << std::endl;
                printSubtree(out,root->right, newPrefix);
        }

        if (hasLeft)
        {
                out << (hasRight ? prefix : "") << "└── " << root->left->value << std::endl;
...
out<<std::endl;
out<<"The Binary Tree :"<<std::endl;
        if (btree._root == NULL)
        {
                return out;
        }

        out << btree._root->value << std::endl;
...
template<typename T>
T BinaryTree<T>::GetNextItem(std::queue<T> printQueue) {
    T value = printQueue.front();  // Top, peek, front
    printQueue.pop();
    return value;
}
template<typename T> 
void BinaryTree<T>::cleanQ(std::queue<Node*> printQueue){
...
template<typename T>
void BinaryTree<T>::PlacePreOrder(Node **node,std::queue<Node*> &printQueue) {
    if (*node == nullptr){
        return;}
...
template<typename T>
void BinaryTree<T>::PlacePostOrder(Node **node,std::queue<Node*> &printQueue) {
    if (*node == nullptr){
        return;}
...
template<typename T>
void BinaryTree<T>::PlaceInOrder(Node **node,std::queue<Node*> &printQueue) {
    if (*node == nullptr){
        return;}
...
BinaryTree<T>::iterator::iterator(Node* n,Order traverseOrder){
 ...
 template <typename T>
    ...
    BinaryTree<T> &BinaryTree<T>::add_left(T val, T node)
   {
    ...
    {
      if (_root == nullptr){throw std:: invalid_argument("there is no root it this tree");}
       Node *n = FindNode(_root, val);
        if (n == nullptr) {
            throw std::invalid_argument(
