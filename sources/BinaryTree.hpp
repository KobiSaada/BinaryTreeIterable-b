...
Node& operator=(const Node& other)
{
    ...
    Node * l = left;
    left = new Node(*other.left);
    delete l;//call to discractur

    Node * r= right;
    right = new Node(*other.right);
    delete r;//call to discractur
    ...
    unsigned int get_depth() const{
        ...
        T* operator->() const {
            ...
            const iterator operator++(int);//posfix
  ...
  //  bool Contains(T value);
    ...
    ~BinaryTree(){
        ...
        BinaryTree<T> & add_right(T node, T r);
         ...
         void Copy_tree(Node *thistree,  Node *othertree) {
